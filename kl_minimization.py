...
def kl_minimization(current_model,good_teacher,batch, device,bt,bad_teacher_model):
...
#challenge's pre trained model for retain set (good teacher)
  ...
  for forget_epoch in range(epoch):
    ...
    for batch in train_set:
        optimizer.zero_grad()
        loss=kl_minimization(unlearnmodel,good_teacher,batch,device,config["bad_teacher"],bad_teacher_model)
    ...
    with torch.no_grad():
        for batch in val_set:
            val_loss=kl_minimization(unlearnmodel,good_teacher,batch,device,config["bad_teacher"],bad_teacher_model)
